...
import org.nlogo.core.{ CompilationOperand, DummyCompilationEnvironment, DummyExtensionManager,
  DummyModuleManager, CompilerException, Femto, StructureResults}
...
class StructureParserTests extends FunSuite {
  ...
  def compileAll(src: String): StructureResults = {
    StructureParser.parseSources(
      tokenizer,
      CompilationOperand(Map("" -> src), new DummyExtensionManager, new DummyModuleManager, new DummyCompilationEnvironment, subprogram = false),
  ...
  test("mutually referrent sources") {
    ...
    val results =
      StructureParser.parseSources(
        tokenizer, CompilationOperand(sources, new DummyExtensionManager, new DummyModuleManager, new DummyCompilationEnvironment, subprogram = false))
