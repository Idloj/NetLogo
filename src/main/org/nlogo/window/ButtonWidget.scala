...
import org.nlogo.awt.ValueImageFilter
...
object ButtonWidget {
  ...
  val FOREVER_ICON_ACTIVE: ImageIcon = image("/images/forever-active.gif")

  object ButtonType {

    // the 4 possible button types
    val ObserverButton = ButtonType("observer", classOf[Observer], img = None, activeImg = None)
    ...
    def lightImage(image: ImageIcon) = new ImageIcon(java.awt.Toolkit.getDefaultToolkit.createImage(
      new FilteredImageSource(image.getImage.getSource, new ValueImageFilter(1.6))))

    private def apply(headerCode:String, agentClass:Class[_ <: Agent], imagePath: String): ButtonType = {
      val img = image(imagePath)
      new ButtonType(headerCode, agentClass, Some(img), Some(lightImage(img)))
    }
  ...
  case class ButtonType(name: String, agentClass:Class[_ <: Agent],
                        img:Option[ImageIcon], activeImg:Option[ImageIcon]){
    def img(active: Boolean): Option[ImageIcon] = if (active) activeImg else img
...
class ButtonWidget(random:MersenneTwisterFast) extends JobWidget(random)
  ...
  override def paintComponent(g: Graphics) {
    ...
    def paintButtonRectangle(g: Graphics) {
      ...
      def renderImages(g: Graphics, active: Boolean) {
        def maybePaintForeverImage() {
          if (forever) {
            val image = if (active) FOREVER_ICON_ACTIVE else FOREVER_ICON
            image.paintIcon(this, g, getWidth() - image.getIconWidth - 4, getHeight() - image.getIconHeight - 4)
          }
        }
        def maybePaintAgentImage() {
          buttonType.img(active).map(_.paintIcon(this, g, 3, 3))
        }
